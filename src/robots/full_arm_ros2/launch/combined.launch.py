...
def generate_launch_description():
    ...
    # Combined URDF (Xacro)
    urdf_file = PathJoinSubstitution([FindPackageShare("full_arm_ros2"), "urdf", "full_arm_ros2.urdf.xacro"])
    robot_description_content = Command([FindExecutable(name="xacro"), " ", urdf_file, " sim:=", sim])
    robot_description = {"robot_description": robot_description_content}

    # Combined Controllers YAML
    robot_controllers = PathJoinSubstitution([FindPackageShare("full_arm_ros2"), "config", "combined_controllers.yaml"])
    ...
    # Delay spawners until joint_state_broadcaster finishes
    delay_diffbot_controller_spawner = RegisterEventHandler(
        event_handler=OnProcessExit(
            target_action=joint_state_broadcaster_spawner,
            on_exit=[diffbot_controller_spawner],
        )
    )
    ...
    nodes = [
        ...
        delay_diffbot_controller_spawner,
